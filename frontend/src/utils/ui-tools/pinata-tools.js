...
export const urlPocket = configs[CONFIG].pocketURL;
...
export const fetchJSONByCID = async ({ id, table, expand }) => {
  ...
  try {
    const record = expand
      ? await clientPocket.records.getOne(table, id, {
          expand,
        })
      : await clientPocket.records.getList(table, 1, 1, {
          filter: `id = "${id}"`,
        });

    return expand ? record : record.items?.[0];
  } catch (error) {
    console.error(
