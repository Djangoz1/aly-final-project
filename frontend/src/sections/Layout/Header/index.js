...
export const Header = ({ setter, isOpen }) => {
  ...
  return (
    <>
      <motion.div
        className={`flex w-full  relative -z-1 box-border  flex-col min-h-[5vh] `}
      >
        <motion.div className="relative   flex z-100 w-full py-5 px-5   box-border items-center justify-between backdrop-blur">
          ...
          <MyProgress style="progress bg2 opacity-50  mr-5 w-[97%]  absolute bottom-0 " />
        </motion.div>
        <>
          ...
          <motion.div
            ...
            className={`h-[100vh]  max-w-full backdrop-blur  absolute right-0    bg-white/60 font2 top-0        flex flex-col items-start   ${
              isClicked ? "w-[25vw]" : "w-0"
            }`}
          >
            <div className="px-10 flex relative border-box flex-col h-full py-5 w-full  ">
              <button
                onClick={() => setIsClicked(!isClicked)}
